...
use x_editor::annotated_ast::{AnnotatedExpr, AnnotatedValueDef};
use x_editor::content_addressing::{ContentHash, ContentRepository};
use x_editor::namespace::{
    NameBinding, Namespace, NamespacePath, Visibility as NamespaceVisibility,
};
use x_parser::{ast::*, Symbol};
...
impl TestDiscovery {
    ...
    pub fn discover_by_pattern(&self, namespace: &Namespace, pattern: &str) -> Result<TestSuite> {
        ...
        let filtered_tests: Vec<TestCase> = all_tests
            .tests
            .into_iter()
    ...
    fn check_if_test(
        ...
        } = member
        {
            ...
            if self.is_test_function_by_name(name)
                || (type_scheme
                    .as_ref()
                    .map_or(false, |ts| self.is_test_return_type(&ts.body)))
            {
    ...
    fn create_dummy_test_function(
        ...
        AnnotatedValueDef {
            ...
            body: AnnotatedExpr {
                expr: Expr::Literal(
                    Literal::Bool(true),
                    Span::new(FileId(0), ByteOffset(0), ByteOffset(1)),
                ),
                inferred_type: None,
...
mod tests {
    ...
    use x_parser::Purity;
    use x_parser::{span::ByteOffset, FileId, Span};
