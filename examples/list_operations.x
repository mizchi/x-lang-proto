...
---
@function filter
@param {pred: a -> Bool} Predicate function
@param {list: List[a]} The input list
...
---
@function fold_left
@param {f: b -> a -> b} The folding function
...
test "list operations" {
    ...
    // Test filter
    let filtered = filter (fun x -> x > 3) nums in
    let expected_filtered = [4; 5] in
    filtered == expected_filtered &&
    filter (fun x -> x < 0) nums == [] &&
    
    // Test fold_left
    fold_left (fun acc x -> acc + x) 0 nums == 15 &&
    let reversed_via_fold = fold_left (fun acc x -> x :: acc) [] nums in
