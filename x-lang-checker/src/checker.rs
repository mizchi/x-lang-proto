...
use x_lang_parser::{CompilationUnit, Module, Item, ValueDef, TypeDef, Symbol};
...
impl TypeChecker {
    ...
    fn check_type_def(&mut self, type_def: &TypeDef) {
        ...
        match &type_def.definition {
            x_lang_parser::TypeDefinition::Record(fields) => {
            ...
            x_lang_parser::TypeDefinition::Variant(variants) => {
            ...
            x_lang_parser::TypeDefinition::Alias(aliased_type) => {
    ...
    fn check_effect_def(&mut self, _effect_def: &x_lang_parser::EffectDef) {
        // TODO: Implement effect definition checking
    }

    fn check_handler_def(&mut self, _handler_def: &x_lang_parser::HandlerDef) {
        // TODO: Implement handler definition checking
    }

    fn check_interface_def(&mut self, _interface_def: &x_lang_parser::ComponentInterface) {
        // TODO: Implement interface definition checking
    }

    fn check_import_def(&mut self, _import_def: &x_lang_parser::ImportDef) {
        // TODO: Implement import definition checking
    }

    fn check_export_def(&mut self, _export_def: &x_lang_parser::ExportDef) {
        // TODO: Implement export definition checking
    }

    fn check_import(&mut self, _import: &x_lang_parser::ImportDef) {
        // TODO: Implement import checking
    }

    fn check_export(&mut self, _export: &x_lang_parser::ExportDef) {
        // TODO: Implement export checking
    }

    /// Helper methods
    fn check_type_annotation(&self, inferred: &Type, annotation: &x_lang_parser::Type) -> Result<(), TypeError> {
    ...
    fn check_type_well_formed(&self, _typ: &x_lang_parser::Type) -> Result<(), TypeError> {
    ...
    fn convert_parser_type_to_checker_type(&self, parser_type: &x_lang_parser::Type) -> Type {
        match parser_type {
            x_lang_parser::Type::Unit => Type::Unit,
            x_lang_parser::Type::Bool => Type::Bool,
            x_lang_parser::Type::Int => Type::Int,
            x_lang_parser::Type::Float => Type::Float,
            x_lang_parser::Type::String => Type::String,
            x_lang_parser::Type::Char => Type::Char,
            x_lang_parser::Type::List(inner) => {
                Type::App(Box::new(Type::Con(Symbol::new("List"))), vec![self.convert_parser_type_to_checker_type(inner)])
            }
            x_lang_parser::Type::Option(inner) => {
                Type::App(Box::new(Type::Con(Symbol::new("Option"))), vec![self.convert_parser_type_to_checker_type(inner)])
            }
            x_lang_parser::Type::Result(ok, err) => {
                Type::App(
            ...
            x_lang_parser::Type::Named(name) => Type::Con(*name),
            x_lang_parser::Type::Variable(var) => Type::Var(crate::types::TypeVar(*var)),
            _ => Type::Unknown,
        }
    }
...
mod tests {
    use super::*;
    use x_lang_parser::{parse_source, SyntaxStyle, FileId};
