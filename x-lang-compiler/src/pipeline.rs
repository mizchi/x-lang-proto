...
use x_lang_parser::{parse_with_metadata, SyntaxStyle, FileId};
use x_lang_checker::type_check;
...
impl CompilationPipeline {
    ...
    fn run_parse_stage(
        &self,
        source: &str,
    ) -> Result<PipelineResult<x_lang_parser::CompilationUnit>, CompilerError> {
    ...
    fn run_typecheck_stage(
        &self,
        ast: &x_lang_parser::CompilationUnit,
    ) -> Result<PipelineResult<x_lang_checker::CheckResult>, CompilerError> {
        let start = Instant::now();
    ...
    fn count_ast_nodes(&self, ast: &x_lang_parser::CompilationUnit) -> usize {
