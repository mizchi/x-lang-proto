//! EffectLang Compiler
//! 
//! This crate provides code generation and compilation functionality for EffectLang.
...
use effect_lang_parser::{CompilationUnit, SyntaxStyle};
use effect_lang_checker::{type_check, CheckResult};
...
pub struct CompilerDiagnostic {
    ...
    pub span: Option<effect_lang_parser::Span>,
}
...
pub enum CompilerError {
    #[error("Parse error: {0}")]
    Parse(#[from] effect_lang_parser::ParseError),
...
pub mod convenience {
    ...
    pub fn type_check_source(source: &str) -> Result<CheckResult, CompilerError> {
        use effect_lang_parser::{parse_source, FileId};
...
mod tests {
    ...
    fn test_compiler_builder() {
        let compiler = CompilerBuilder::new()
            .syntax_style(SyntaxStyle::OCaml)
            .optimization_level(2)
            .debug_info(true)
            .build();

        assert_eq!(compiler.config.syntax_style, SyntaxStyle::OCaml);
    ...
    fn test_convenience_functions() {
        ...
        // Test parsing
        let result = convenience::parse_source_only(source, SyntaxStyle::OCaml);
        assert!(result.is_ok());
