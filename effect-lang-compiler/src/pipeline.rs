...
use effect_lang_parser::{parse_with_metadata, SyntaxStyle, FileId};
use effect_lang_checker::type_check;
...
impl CompilationPipeline {
    ...
    fn run_parse_stage(
        &self,
        source: &str,
    ) -> Result<PipelineResult<effect_lang_parser::CompilationUnit>, CompilerError> {
    ...
    fn run_typecheck_stage(
        &self,
        ast: &effect_lang_parser::CompilationUnit,
    ) -> Result<PipelineResult<effect_lang_checker::CheckResult>, CompilerError> {
        let start = Instant::now();
    ...
    fn count_ast_nodes(&self, ast: &effect_lang_parser::CompilationUnit) -> usize {
...
mod tests {
    ...
    fn test_ast_node_counting() {
        ...
        use effect_lang_parser::{parse_source, FileId, SyntaxStyle};
        let source = "let x = 42\nlet y = true";
        let file_id = FileId::new(0);
