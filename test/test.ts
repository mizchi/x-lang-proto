...
import { BinarySerializer, calculateHash, generateContentHash } from '../src/serializer.js';
...
test('複雑なS式の差分', () => {
  ...
  const sexp1 = parser.parse('(defun factorial (n) (if (= n 0) 1 (* n (factorial (- n 1)))))');
  const sexp2 = parser.parse('(defun factorial (n) (if (<= n 1) 1 (* n (factorial (- n 1)))))');
